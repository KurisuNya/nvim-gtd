...
function gtd.register_source(name, source)
	source.get_position_encoding_kind = source.get_position_encoding_kind
		or function()
...
function gtd.exec(params, config)
	...
	Async.run(function()
		...
		for _, source_configs in ipairs(config.sources) do
			...
			for _, source_config in ipairs(kit.to_array(source_configs)) do
				...
				if source then
					...
					locations = locations:catch(function()
						return {}
					end)
		...
		:next(function(
			locations --[[ @as gtd.kit.LSP.LocationLink[] ]]
		)
			if #locations == 0 then
...
function gtd._normalize(locations, context, position_encoding_kind)
	...
	for _, location in ipairs(locations) do
		...
		local start =
			Position.to(context.text, location.targetRange.start, position_encoding_kind, LSP.PositionEncodingKind.UTF8)
